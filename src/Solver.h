...
//                , locally_owned_dofs(locally_owned_dofs_per_proc[problem->get_mesh()->get_this_mpi_processes()])
            ...
            locally_owned_dofs_per_proc = DoFTools::locally_owned_dofs_per_subdomain(
                    problem->get_mesh()->get_dof_handler());
            locally_owned_dofs = locally_owned_dofs_per_proc[problem->get_mesh()->get_this_mpi_processes()];
...
//            << locally_owned_dofs << endl;
            ...
            u.reinit(locally_owned_dofs, problem->get_mesh()->get_mpi_communicator());
            du.reinit(locally_owned_dofs, problem->get_mesh()->get_mpi_communicator());
            system_rhs.reinit(locally_owned_dofs, problem->get_mesh()->get_mpi_communicator());
            u = 0;
            du = 0;
        };
...
//            du_constraints.template distribute(du);
    ...
    void Solver<dim>::output_results() const {
        ...
        if (problem->get_mesh()->get_this_mpi_processes() == 0) {
            ...
            data_out.add_data_vector(u, "displacement", DataOut<dim>::type_dof_data, data_component_interpretation);
            ...
            for (const auto &out_item: problem->get_stage().get_scalar_outputs()) {
                scalar_values.push_back(problem->get_mesh()->project_scalar_qp_field(out_item.first, out_item.second));
            ...
            for (const auto &out_item: problem->get_stage().get_vector_outputs()) {
                vector_values.push_back(problem->get_mesh()->project_vector_qp_field(out_item.first, out_item.second));
