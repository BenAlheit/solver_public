...
//        u.update_ghost_values();
    ...
    template<unsigned int dim>
    void Solver<dim>::assemble_system_old(bool init_step, bool init_solve) {
        system_matrix = 0.0;
        system_rhs = 0.0;
        ...
        for (const auto &cell: problem->get_mesh()->get_dof_handler().active_cell_iterators()) {
            if (cell->subdomain_id() == problem->get_mesh()->get_this_mpi_processes()) {
                cell_matrix = 0;
                cell_rhs = 0;
                cell->get_dof_indices(local_dof_indices);
                for (const auto &i_dof: problem->get_mesh()->get_dof_range())
                    local_u[i_dof] = localized_solution[local_dof_indices[i_dof]];


                for (const auto &qp: problem->get_mesh()->get_qp_range()) {
                    JxW = problem->get_mesh()->get_JxW(cell->level(), cell->index(), qp);
                    Grad_u = problem->get_mesh()->grad_field(cell->level(), cell->index(), qp, local_u);
                    F = I + Grad_u;
                    F_inv = invert(F);
                    qp_state = problem->get_mesh()->get_state(cell->level(), cell->index(), qp);

                    qp_state->F_n1 = F;

                    if (not init_step or init_solve) {
                        material = problem->get_mesh()->get_material(cell->material_id());
                    ...
                    ref_grad_phis = problem->get_mesh()->get_grad_shape_fns(cell->level(), cell->index(), qp);
...
//            u.update_ghost_values();
    ...
    template<unsigned int dim>
    void Solver<dim>::output_results_old() const {
        const Vector<double> localized_solution(u);
...
//            for (const auto &out_item: problem->get_stage().get_scalar_outputs()) {
//                scalar_values.push_back(
//                        problem->get_mesh()->project_scalar_qp_field(out_item.first, out_item.second));
//                scalar_names.push_back(OutputFlags::to_string(out_item.first));
//            }
//
//            for (const auto &out_item: problem->get_stage().get_n_scalar_outputs()) {
//                for (unsigned int i = 0; i < out_item.first.second; i++) {
//                    scalar_values.push_back(problem->get_mesh()->project_n_scalar_qp_field(out_item.first.first,
//                                                                                           out_item.second,
//                                                                                           i));
//                    scalar_names.push_back(OutputFlags::to_string(out_item.first.first, i));
//                }
...
//            for (const auto &out_item: problem->get_stage().get_vector_outputs()) {
//                vector_values.push_back(
//                        problem->get_mesh()->project_vector_qp_field(out_item.first, out_item.second));
//                vector_names.push_back(OutputFlags::to_string<dim>(out_item.first));
//            }
//
//            for (const auto &out_item: problem->get_stage().get_n_vector_outputs()) {
//                for (unsigned int i = 0; i < out_item.first.second; i++) {
//                    vector_values.push_back(problem->get_mesh()->project_n_vector_qp_field(out_item.first.first,
//                                                                                           out_item.second,
//                                                                                           i));
//                    vector_names.push_back(OutputFlags::to_string<dim>(out_item.first.first, i));
//                }
//            }
...
//            for (const auto &out_item: problem->get_stage().get_tensor_outputs()) {
//                tensor_values.push_back(
//                        problem->get_mesh()->project_tensor_qp_field(out_item.first, out_item.second));
//                tensor_names.push_back(OutputFlags::to_string<dim>(out_item.first));
//            }
//
//            for (const auto &out_item: problem->get_stage().get_n_tensor_outputs()) {
//                for (unsigned int i = 0; i < out_item.first.second; i++) {
//                    tensor_values.push_back(problem->get_mesh()->project_n_tensor_qp_field(out_item.first.first,
//                                                                                           out_item.second,
//                                                                                           i));
//                    tensor_names.push_back(OutputFlags::to_string<dim>(out_item.first.first, i));
//                }
//            }
...
//                        name = name + "_el_wise";
            ...
            data_out.build_patches();
//            string rel_vtu_name = problem->get_name() + "-t-" + to_string(problem->get_time()->get_timestep()) + ".vtu";
            string rel_vtu_name = problem->get_name()
                                  + "-s-" + to_string(problem->get_time()->get_stage())
                                  + "-t-" + to_string(problem->get_time()->get_timestep()) + ".vtu";
...
//                                  + "-t-" + to_string(problem->get_time()->get_timestep()) + ".vtu";
            ...
            static vector<pair<double, string>> times_and_names;
            times_and_names.emplace_back(pair<double, string>(problem->get_time()->current(), rel_vtu_name));
//            string pvd_name = output_dir_name + problem->get_name() + "-np-" +
//                              to_string(problem->get_mesh()->get_n_mpi_processes()) + ".pvd";
            string pvd_name = output_dir_name + problem->get_name() + ".pvd";
            ofstream pvd_output(pvd_name);
            DataOutBase::write_pvd_record(pvd_output, times_and_names);
