#ifndef SOLVER_CUBEWITHSPHERE_H
#define SOLVER_CUBEWITHSPHERE_H
...
class CubeWithSphere {
public:
    explicit CubeWithSphere(unsigned int n_refinements = 2);
...
template<unsigned int dim>
CubeWithSphere<dim>::CubeWithSphere(unsigned int n_refinements):
        n_refinements(n_refinements) {

    iota(range.begin(), range.end(), 0);
    read_and_refine_mesh();
//    string name = "elastic-cube-with-particle";
    string name = "elastic-cube-with-particle-shear";
...
//    const double mult = 1;
    ...
    Fix<dim> left_fix(/*boundary_id*/1);

//    DirichletBoundaryCondition<dim> right( /*boundary_id*/ 4,
...
template<unsigned int dim>
void CubeWithSphere<dim>::read_and_refine_mesh() {
    GridIn<dim> grid_in;
