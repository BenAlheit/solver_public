...
#ifndef SOLVER_POLYCRYSTALIMPPBC_H
#define SOLVER_POLYCRYSTALIMPPBC_H
...
class PolycrystalIMPPBC {
public:
    explicit PolycrystalIMPPBC(unsigned int n_refinements = 0);
...
template<unsigned int dim>
PolycrystalIMPPBC<dim>::PolycrystalIMPPBC(unsigned int n_refinements):
        n_refinements(n_refinements) {

    iota(range.begin(), range.end(), 0);
    read_and_refine_mesh();

    string name = "polycrystal-imp-psc-ref-"+ to_string(ref);
    const double kappa = 44.12e3;
    const double mu = 16.92e3;
    const double mult = 1e1;
    const double mult_p = 1e2;
    ...
    for (unsigned int i = 0; i<n_grains; i++ ) {
        ...
        materials[/*material id*/ i+2] = new NeoHookean<dim>(/*material id*/ i+2, mult * kappa, mult * mu);
    }
...
template<unsigned int dim>
void PolycrystalIMPPBC<dim>::read_and_refine_mesh() {
    ifstream input_file("../src/examples/polycrystal-imp/polycrystal-imp-" + to_string(ref) + ".dtri");
    {
...
#endif //SOLVER_POLYCRYSTALIMPPBC_H
