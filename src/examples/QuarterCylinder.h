...
#ifndef SOLVER_QUARTERCYLINDER_H
#define SOLVER_QUARTERCYLINDER_H
...
public:
    explicit QuarterCylinder(unsigned int n_refinements = 2);
...
template<unsigned int dim>
QuarterCylinder<dim>::QuarterCylinder(unsigned int n_refinements) : n_refinements(n_refinements) {
    iota(range.begin(), range.end(), 0);

    make_mesh();
    string name = "quarter-cylinder";
    ...
    dbcs.push_back(new DirichletBoundaryCondition<dim>( /*boundary_id*/ 4,
            ...
            /*magnitudes*/ vector<double>({pull, 0, 0})));
...
template<unsigned int dim>
void QuarterCylinder<dim>::make_mesh() {
...
#endif //SOLVER_QUARTERCYLINDER_H
